#include <Arduino.h>
#include "BluetoothSerial.h" // Assuming this is correct for your ESP32 Bluetooth
#include <CytronMotorDriver.h> // Include the library
...
const int LEFT_DIR_PIN = 17; // Connect to DIG1 (or IN1) on MDDS30 (ensure this is an available ESP32 GPIO)

// Right Motor Pins
const int RIGHT_PWM_PIN = 19; // Connect to AN2 on MDDS30
const int RIGHT_DIR_PIN = 16; // Connect to DIG2 (or IN2) on MDDS30 (ensure this is an available ESP32 GPIO)
...
CytronMotorDriver motorLeft(PWM_DIR, LEFT_PWM_PIN, LEFT_DIR_PIN);
CytronMotorDriver motorRight(PWM_DIR, RIGHT_PWM_PIN, RIGHT_DIR_PIN);
...
int processAxis(int rawValue, int centerValue) {
  ...
  // The map function can be used, or a linear calculation
  if (processedValue > 0) {
